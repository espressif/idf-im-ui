...
export function runInstallationStatusTest({
  ...
  describe(`${id}- EIM Installation Status tests |`, function () {
    ...
    it("2- 'eim list' output should include a status label for each installation", async function () {
      ...
      // After a successful install every entry should show the "ok" label
      // (the localized label for status="finished"; see list.status.finished
      // in src-tauri/locales/app.yml). We check that at least one of the
      // displayed status labels appears in the output.
      expect(
        testRunner.output.toLowerCase(),
        "eim list output does not include any status label"
      ).to.match(/ok|in.?progress|failed|being.?repaired|broken/i);
    ...
    it("3- Interactive 'eim select' prompt should show status in brackets", async function () {
      ...
      // The prompt list items are formatted as "name [status]", where
      // [status] is the localized label (e.g. "ok" for finished entries).
      expect(
        testRunner.output,
    ...
    it("5- Fixing a 'failed' entry should transition status to 'finished'", async function () {
      ...
      try {
        ...
        testRunner = new CLITestRunner();
        await testRunner.start();
        testRunner.callEIM(pathToEIM, ["fix", "--path", targetEntry.path]);

        const fixStarted = await testRunner.waitForOutput("Fixing", 10000);
        expect(fixStarted, "eim fix did not start").to.be.true;

        // Wait for the final fix-completion message emitted by the CLI fix
        // command after the wizard finishes and eim_idf.json is updated to
        // status="finished" (see fix.success / fix.ready in locales).
        // The generic word "successfully" would also match the mid-fix
        // "Python environment installed successfully" line, which fires
        // while the entry is still in `being_repaired` status.
