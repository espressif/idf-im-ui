...
is_sourced() {
    case "$0" in
        -*) return 0 ;;           # Shell names starting with - (like -bash, -zsh)
        */sh|*/bash|*/zsh|*/dash|*/fish) return 0 ;;  # Shell executables
        sh|bash|zsh|dash|fish) return 0 ;;            # Shell names without path
        *) return 1 ;;            # Likely a script path
    esac
}
