...
function parse_cmake_version
    ...
    while read -l line
        ...
        if string match -q "set(IDF_VERSION_MAJOR *" "$line"
            set major (string match -r -o 'set\(IDF_VERSION_MAJOR\s*(\d+)' "$line" | head -1)
        else if string match -q "set(IDF_VERSION_MINOR *" "$line"
            set minor (string match -r -o 'set\(IDF_VERSION_MINOR\s*(\d+)' "$line" | head -1)
        end
    end < "$cmake_file"
...
function print_env_variables
    ...
    for pair in $ENV_VAR_PAIRS
        if test -n "$pair"
            set key (string split -n1 : $pair)[1]
            set value (string split -n2 : $pair)[2]
            printf '%s=%s\n' "$key" "$value"
        end
    end
end
...
function add_to_path
    set -gx PATH {{addition_to_path}} $PATH
...
set venv_default "{{idf_python_env_path_escaped}}"
...
if test -n "$_EIM_BIN" -a -x "$_EIM_BIN"
