use crate::{
    command_executor::execute_command, idf_config::{IdfConfig, IdfInstallation}, idf_tools::{read_and_parse_tools_file}, single_version_post_install, version_manager::get_default_config_path
};
...
pub fn parse_esp_idf_json(idf_json_path: &str) -> Result<()> {
    ...
    for (_key, value) in config.idf_installed {
        ...
        let python_env = python.clone().find("bin").map(|index| python[..=index-1].to_string());
        ...
        let python = match std::env::consts::OS {
            "windows" => PathBuf::from(python_env.unwrap()).join("Scripts").join("python.exe"),
            _ => PathBuf::from(python_env.unwrap()).join("bin").join("python"),
        };
