...
pub fn get_esp_idf(
    ...
    let had_preexisting_content = match std::fs::read_dir(&dest_path) {
      Ok(mut it) => it.next().is_some(),
      Err(_) => false, // path missing or unreadable — treat as "we own it"
    };
    ...
    let gix_attempt = clone_repository(
      CloneOptions {
        ...
        reference: rebuild_reference(&clone_options.reference),
...
/// Clone a `GitReference` value without requiring `Clone` on the enum itself.
fn rebuild_reference(reference: &GitReference) -> GitReference {
    match reference {
        GitReference::Branch(b) => GitReference::Branch(b.clone()),
        GitReference::Tag(t) => GitReference::Tag(t.clone()),
        GitReference::Commit(c) => GitReference::Commit(c.clone()),
        GitReference::None => GitReference::None,
    }
}
