                    ...
                    if config.idf_installed.is_empty() {
                        ...
                        for version in config.idf_installed {
                            if version.id == config.idf_selected_id {
                                ...
                                println!(
                                    "{}",
                                    t!("list.version", name = version.name, path = version.path)
                                );
                            }
                        }
                        Ok(())
                ...
                match idf_im_lib::version_manager::list_installed_versions() {
                    Ok(versions) => {
                        if versions.is_empty() {
                            ...
                            let options = versions.iter().map(|v| v.name.clone()).collect();
                            let version =
                                match helpers::generic_select(&t!("rename.prompt"), &options) {
                ...
                match idf_im_lib::version_manager::list_installed_versions() {
                    Ok(versions) => {
                        if versions.is_empty() {
                            ...
                            let version_path =
                                match helpers::generic_select(&t!("fix.prompt"), &options) {
                                    Ok(selected) => selected,
                                    Err(err) => {
                                        error!("Error: {}", err);
                                        return Err(anyhow::anyhow!(err));
